...
public class DateTab extends LinearLayout {
    ...
    private ArrayList<ObjectJadwal> jadwals = db.getAllJadwal();
    ...
    public DateTab(Context context, AttributeSet attrs) {
        super(context, attrs);
//        Log.d("cingire2",Integer.toString(db.getAllJadwal().get(0).getSesi()));
        ...
        ArrayList<Integer> sesi = new ArrayList<>();
        for(int i = 0; i < 5; i++) {
            for(int k=0;k<jadwals.size();k++){
                s = getImageViewList(i);
                sesi.add(jadwals.get(k).getSesi());
                if(konten[i].getDate().equals(jadwals.get(k).getTanggal())){
                    for (int ss : sesi) {
                        for (int j = 0; j < 4; j++) {
                            if (ss == (j + 1)) {
                                if (todayno == (i+2)) {
